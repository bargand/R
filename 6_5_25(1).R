...
claimed_mean <- 500        # Claimed mean weight (mu_0)
sample_mean <- 495         # Sample mean weight (x_bar)
std_dev <- 20              # Known population standard deviation (sigma)
...
z_stat <- -1.581; alpha <- 0.05; z_critical <- qnorm(alpha)
...
legend(4.5, 0.4, legend=c("Normal Curve", "Z-statistic", "Critical Value", "Rejection Region"), col=c("blue", "red", "black", rgb(1,0,0,0.3)), lty=c(1, 2, 2, NA), fill=c(NA, NA, NA, rgb(1,0,0,0.3)), border=NA, bty="n")̥